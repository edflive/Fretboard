document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateFretboardImage() {
    const fretCount = parseInt(document.getElementById('fret-count').value, 10);
    const fretboardImage = document.getElementById('fretboard-image');
    const svg = document.getElementById(`fretboard${fretCount}`);

    if (svg) {
        // Clone le SVG pour ne pas modifier l'original
        const clonedSvg = svg.cloneNode(true);
        clonedSvg.style.display = 'block'; // ✅ Supprime le display: none

        // Insère le SVG cloné dans la div
        fretboardImage.innerHTML = clonedSvg.outerHTML;
    }
  }
